...
% %-----------------------------------------------------------------------------
% %TP2_6/ Ahora con un observador que solo puede ver el desplazamiento,
% MODIFIQUE LOS POLOS A LAZO CERRADO PORQUE NO FUNCIONABA SI NO,CONSULTAR
% SI ESTA BIEN.
% %----------------------------------------------------------------------------
% clc;clear all;
% m=.1;Fricc=0.1; long=0.6;g=9.8;Ma=.5;
...
% %CONTROLADOR Ubicación de los polos de lazo cerrado en mui :
% %mui(1)=-2;mui(2)=-2; mui(3)=-2+1i;mui(4)=conj(mui(3));
% mui(1)=-20;mui(2)=-50; mui(3)=-.5 + 0.5i;mui(4)=conj(mui(3));       %<----NUEVO K PORQUE NO FUNCIONABA EL SIN OBSERVADOR
% alfa_i=conv(conv(conv([1 -mui(3)],[1 -mui(4)]),[1 -mui(2)]),[1 -mui(1)]);
...
%  %Variables del sistema no lineal
%  estado=[p(i); p_p(i); alfa(i); omega(i)];
%  u(i)=-K*x_hat+Gj*ref_p; %Con Observador
%  
%  %Sistema no lineal
%  p_pp=(1/(Ma+m))*(u(i)-m*long*tita_pp*cos(alfa(i))+m*long*omega(i)^2*sin(alfa(i))- Fricc*p_p(i));
%  tita_pp=(1/long)*(g*sin(alfa(i))-p_pp*cos(alfa(i)));
...
%-----------------------------------------------------------------------------
%TP2_7 / Controlador con distintos angulos de inicio para probar el limite
%del sistema
%----------------------------------------------------------------------------
% clc;clear all;
% m=.1;Fricc=0.1; long=0.6;g=9.8;Ma=.5;
...
% %-----------------------------------------------------------------------------
% %TP2_7 / Controlador con integrador funciona mucho mejor pero el angulo no
...
% 
%-----------------------------------------------------------------------------
%TP2_8 / Controlador con integrador y observador. No esta en el punto de
%funcionamiento adecuado, hay que acomodarlo pero la simulacion esta bien
%creo.
%----------------------------------------------------------------------------
...
%Ubicación de los polos de lazo cerrado en mui:
% mui(1)=-0.2;mui(2)=-0.2; mui(3)=-10 + 0.01i;mui(4)=conj(mui(3));mui(5)=-1;
mui(1)=-25;mui(2)=-25; mui(3)=-10 + 5i;mui(4)=conj(mui(3));mui(5)=-1;
alfa_ia=conv(conv(conv(conv([1 -mui(3)],[1 -mui(4)]),[1 -mui(2)]),[1 -mui(1)]),[1 -mui(5)]);
Ka=(alfa_ia(2:6)-c_ai(2:6))*inv(Mat_Ta);
eig(Mat_Aa-Mat_Ba*Ka)
