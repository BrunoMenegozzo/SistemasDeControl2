%Motor DC TP3: El codigo comienza en la linea 129.
%-------------------------------------------------------------------
%Motor DC DLQR sin integrador del error. No ajusta la referencia negativa correctamente.
%-------------------------------------------------------------------
...
% % %Funcional de costos
%  %
%  Q=diag([1 150000 10]);R=3.2e2; %Ts=1e-5 / Con C es [1 0 0].
%  %Q=diag([1e4 5e8 1e4]);R=8e4; %Ts=1e-4/ Con C es [0 1 0].
%  %Q=diag([1 10 50]);R=1; %Ts=1e-4
...
% plot(t,x3(1:numel(t)),color);title('w');xlabel('Tiempo [Seg.]');
% 
...
% 
% figure(2);
% semilogy(t,Mat_J(1:numel(t)),color);grid on;title('Modelo lineal','FontSize',TamanioFuente);
